...
def visits_and_recent_login(request):
    if request.method == 'GET':
        return Response(GeneralStatistics.visits_and_recent_login())

@api_view(['GET'])
def total_visits(request):
    if request.method == 'GET':
        return Response(GeneralStatistics.total_visits())

@api_view(['GET'])
def visits_per_month(request):
    if request.method == 'GET':
        return Response(GeneralStatistics.visits_per_month())
