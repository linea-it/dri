...
Ext.define('Target.view.catalog.Tree', {
  ...
  viewRecord: function (record) {
    ...
    if (record.get('tableExist') == true && record.get('leaf') == true) {
  ...
  getTooltipName: function (value, meta, record) {
    ...
    var data = {
      ...
      tableExist: record.data.tableExist,
    };
    ...
    tpl = new Ext.XTemplate(
      ...
      '<p><span><b>Proccess: </b></span>{epr_original_id}</p>',
      '</tpl>',
      "<tpl if='tbl_size != null'>",
      '<p><span><b>Size: </b></span>{tbl_size}</p>',
      '</tpl>',
      "<tpl if='!tableExist'>",
      '</br><span><b class=color-orange>Warning</b>: ',
      'The table for this product is not currently available or does not exist.</span>',
      '</tpl>',
      '</div>'
    );

    tooltip = tpl.apply(data);

    // So exibe popup se tiver descricao ou numero de processo.
    if (record.get('description') !== '' || record.get('epr_original_id') !== '') {
      if (record.get('leaf')) {
