Ext.define('Target.view.objects.FiltersController', {
  ...
  onSelectProperty: function (combo, property) {
    ...
    // If the property to be filtered is Rating,
    // then close the Reject and disable the checkbox:
    if (property.get('column_name') === '_meta_rating') {
      refs.chkRejected.setExpanded(false);
      refs.chkRejected.setDisabled(true);

      // Else, enable the Reject checkbox:
    } else {
      refs.chkRejected.setDisabled(false);
    }
  ...
  saveFilterSet: function (fn) {
    ...
    if (filterName !== '') {
      ...
      storeFilterSets.each(function (item) {
        if (
          item.data.fst_name == filterName &&
          (!isUpdate || item.data.id != filterSet.get('id'))
        ) {
          nameExists = true;
        }
      });
      ...
      //novo filterset
        ...
        doSave(
          Ext.create('Target.model.FilterSet', {
      ...
      fset.save({
        callback: function (savedRating, operation, success) {
          ...
          if (success) {
            ...
            if (
              storeFilters.updating > 0 ||
              storeFilters.removed.length > 0 ||
              storeFilters.getNewRecords().length > 0
            ) {
              view.setLoading(true);
              storeFilters.save({
                callback: function () {
  ...
  onDeleteFilterSet: function () {
    ...
    Ext.MessageBox.confirm(
      '',
      'The Filter will be deleted. Do you want continue?',
      function (btn) {
        if (btn === 'yes') {
          doRemove();
        }
  ...
  onApplyFilter: function () {
    ...
    if (filterName) {
      //filterSet && filterSet.get('fst_name')!='') {
  ...
  onFilterWindow_RadioRejectedChange: function (radio) {
    ...
    if (radio.getValue()) {
      ...
      // If the Reject filter is marked,
      // then remove the Rating property from
      // the select:
      indexRating = contents.find('column_name', '_meta_rating');
      if (indexRating > -1) contents.removeAt(indexRating);

      setTimeout(function () {
        me.viewRenderUIStatus();
      }, 10);
    } else {
      // Otherwise, check if it doesn't already exist
      // and add it again:
      indexRating = contents.find('column_name', '_meta_rating');
      if (indexRating === -1) me.onLoadContents(contents);
    }
  },
  ...
  onFilterWindow_ButtonCopyClick: function () {
    ...
    if (storeFilters.count() > 0 && modelFilterSet) {
      ...
      storeFilters.getData().each(function (item) {
        delete item.data.id;
        if (
          item.data.property_name == '_meta_rating' ||
          item.data.property_name == '_meta_reject'
        ) {
